...
- [ ] Install the recorded lower RC baseline, check for updates, explicitly install the higher staged candidate, and record both versions plus all checksums. Do not put tokens, draft GitHub URLs, or signing material in the staged manifest. Stable binaries never use staging: they check GitHub `releases/latest/download/latest.json` only.
- [ ] After smoke, manually publish the same GitHub draft. An RC remains available through its owner-controlled RC channel; a stable release makes GitHub's `releases/latest/download/latest.json` public. Record the publisher, time, staged baseline/candidate versions, and checksums in release notes.
