      ...
      <section className="settings-view" role="dialog" aria-modal="true" aria-labelledby="settings-heading">
        ...
        <div className="settings-view__body">
          <fieldset disabled={operationBusy}>
            ...
            <label>正文字体<select aria-label="正文字体" value={draft.bodyFont} onChange={(event) => void update({ bodyFont: event.target.value })}><option value="system-ui, sans-serif">系统默认</option><option value="Microsoft YaHei">微软雅黑</option><option value="SimSun">宋体</option><option value="SimHei">黑体</option><option value="KaiTi">楷体</option><option value="serif">Serif</option></select></label>
            <label>代码字体<select aria-label="代码字体" value={draft.codeFont} onChange={(event) => void update({ codeFont: event.target.value })}><option value="ui-monospace, SFMono-Regular, Consolas, monospace">系统等宽</option><option value="Consolas">Consolas</option><option value="Cascadia Code">Cascadia Code</option><option value="Courier New">Courier New</option><option value="SFMono-Regular">SF Mono</option></select></label>
