...
Release candidates and stable releases remain drafts after automated gates. Their temporary configuration puts the owner-controlled, non-secret `RELEASE_STAGING_ENDPOINT/<tag>/latest.json` first, followed by the appropriate public GitHub endpoint (stable `releases/latest`, RC tag-specific). The owner mirrors exact signed draft updater assets to staging for smoke tests, then removes/stops the staged manifest with HTTP 404 before manually publishing the same GitHub draft. Never expose tokens, GitHub draft URLs, or signing material through staging. Updater checks, download/install, and restart are explicit main-window actions through typed services; do not add background or silent update installation.
