      ...
      <section className="settings-view" role="dialog" aria-modal="true" aria-labelledby="settings-heading">
        ...
        <div className="settings-view__body">
          <fieldset disabled={operationBusy}>
            ...
            <label>界面字号<input aria-label="界面字号" type="number" min="12" max="28" value={draft.fontSize} onChange={updateNumberDraft('fontSize')} onBlur={() => void update({ fontSize: draftRef.current.fontSize })} /><span>只调整应用界面，不影响笔记正文</span></label>
            <label className="settings-view__check settings-view__check--described"><input aria-label="每日歌词" type="checkbox" checked={draft.dailyLyrics !== false} onChange={(event) => void update({ dailyLyrics: event.target.checked })} /><span><strong>每日歌词</strong><small>每天在窗口顶部显示一条古代诗词。</small></span></label>
