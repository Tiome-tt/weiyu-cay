...
CREATE TABLE notes (
    ...
    title TEXT NOT NULL,
    folder_id BLOB CHECK (folder_id IS NULL OR (typeof(folder_id) = 'blob' AND length(folder_id) = 16)),
...
CREATE TABLE note_links (
    source_note_id BLOB NOT NULL CHECK (typeof(source_note_id) = 'blob' AND length(source_note_id) = 16),
    target_note_id BLOB NOT NULL CHECK (typeof(target_note_id) = 'blob' AND length(target_note_id) = 16),
...
CREATE TABLE temporary_windows (
    note_id BLOB NOT NULL PRIMARY KEY CHECK (typeof(note_id) = 'blob' AND length(note_id) = 16),
