...
test('multi-selects temporary captures, deletes, undoes, and converts them', async ({ page }) => {
  await page.goto('/')
  await page.getByRole('treeitem', { name: '临时便笺' }).click()
  await expect(page.getByRole('checkbox')).toHaveCount(2)
  await page.getByRole('button', { name: '全选' }).click()
  await page.getByRole('button', { name: '删除所选' }).click()
  await expect(page.getByText('临时便笺为空。')).toBeVisible()
  ...
  await expect(page.getByRole('treeitem', { name: '项目' })).toHaveAttribute('aria-selected', 'true')
})
