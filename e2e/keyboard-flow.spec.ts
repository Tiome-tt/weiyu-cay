...
test('completes the primary application flow with keyboard input only', async ({ page }) => {
  ...
  const newTitle = page.getByRole('textbox', { name: '笔记标题' })
  ...
  await page.keyboard.type('# 键盘流程\n\n')
  await activate(page, page.getByRole('button', { name: '添加内容块' }))
  await activate(page, page.getByRole('menuitem', { name: '插入内部链接' }))
  await expect(page.getByRole('treeitem', { name: '选择链接：用户认证' })).toHaveAttribute('aria-selected', 'true')
  await tabTo(page, page.getByRole('searchbox', { name: '筛选笔记' }))
  await page.keyboard.press('Enter')
  ...
  await activate(page, page.getByRole('button', { name: '阅读视图' }))
  ...
  const keyboardNote = page.locator('.note-card').filter({ hasText: '键盘流程' })
  await expect(keyboardNote).toBeVisible()
  await activate(page, keyboardNote)
  page.once('dialog', (dialog) => dialog.accept())
  const activeNote = page.locator('.note-card[aria-current="true"]')
  await tabTo(page, activeNote)
  await page.keyboard.press('Shift+F10')
  await activate(page, page.getByRole('menuitem', { name: '删除笔记', exact: true }))
  await tabTo(page, page.locator('[role="treeitem"][tabindex="0"]'))
  await page.keyboard.press('Home')
  ...
  await page.keyboard.press('ArrowDown')
  await page.keyboard.press('Enter')
  await activate(page, page.getByRole('button', { name: '导出完整资料库' }))
